Fixes issues where WiFi network discovery fails after the system was suspended. Creates a system service to run at boot to reset the WiFi with each wake. Also restricts some WiFi power saving options to prevent WiFi drops when in use. It will cause a short delay in connection after the system wakes from suspending, but that is far better than the WiFi dying altogether.
